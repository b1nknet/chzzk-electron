...
- **즐겨찾기** — 카드를 마우스 오른쪽 버튼으로 클릭해 즐겨찾기에 추가하면 맨 위의
  별도 그룹에 고정됩니다. 즐겨찾기 채널은 오프라인 숨기기와 무관하게 항상
  표시됩니다.
- **방송 시간순 정렬** — 타이틀바의 ↕ 버튼으로, 라이브 채널은 방송 시작 시간,
  오프라인 채널은 종료 시간 기준으로 정렬합니다 (정렬 중에는 수동 드래그가
  비활성화됩니다).
...
- **드래그 앤 드롭 정렬** 및 채널별 삭제. ⋯ 메뉴에서 확인 대화상자와 함께 모든
  채널을 한 번에 삭제할 수도 있습니다.
- **자동 새로고침** — 30초마다 갱신되며, 새로고침 버튼과 카운트다운이 하나로
  합쳐져 있습니다.
- **오프라인 채널 숨기기** — 방송 중인 채널만 남겨 둘 수 있습니다.
- **모양 조절** — 투명도 슬라이더(100%에서는 완전 불투명), 인터페이스 크기
  조절(− / +), 항상 위에 고정 토글.
- **가져오기 / 내보내기** — 채널 목록과 즐겨찾기를 JSON으로 내보내고 가져올 수
  있습니다 (내보낸 파일 이름에는 타임스탬프가 붙습니다).
...
3. 채널을 클릭하면 브라우저에서 열립니다. 카드를 드래그해 순서를 바꾸거나,
   마우스 오른쪽 버튼으로 클릭해 즐겨찾기에 추가할 수 있습니다.
4. 타이틀바의 보기 전환 버튼으로 목록/그리드를 바꾸고, **↕** 버튼으로 방송
   시간순 정렬을 켜고 끕니다.
5. **⋯** 메뉴에서 항상 위에 고정, 오프라인 채널 숨기기, 모든 채널 삭제,
   가져오기/내보내기, 수동 업데이트 확인을 사용할 수 있습니다. 투명도와 인터페이스
   크기는 하단에서 조절합니다.
